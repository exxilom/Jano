    ...
    document.addEventListener("DOMContentLoaded", () => {
        if (location.href.match(/profile\.php\?edit_password$/)?.input !== undefined) {
            ...
            NEW_PASS2.addEventListener( "keyup", () => {
                ...
                ALERT.innerText = NEW_PASS.value == '' ? 'Se requiere una nueva contraseña' : NEW_PASS.value !== NEW_PASS2.value ? 'Las contraseñas deben coincidir' : '';
            } );
            ...
            BUTTON.addEventListener("click", e => {
                ...
                if ( OLD_PASS.value == "" ) { 
                    ...
                    ALERT.innerText = 'Se requiere la contraseña antigua';
                    e.preventDefault();
                    return;
                }
                //Si el campo d ela contraseña nueva está vacío o si no coincide con la repetición
                    ...
                    ALERT.innerText = NEW_PASS.value == '' ? 'Se requiere una nueva contraseña' : NEW_PASS.value !== NEW_PASS2.value ? 'Las contraseñas no coinciden' : '';
                    e.preventDefault();
                    return;
                }
                //Si la contraseña actual coincide con la nueva
                else if ( OLD_PASS.value == NEW_PASS.value ) {  
                    ALERT.innerText = 'Las contraseñas antiguas y nuevas no pueden coincidir';
                    e.preventDefault();
                    return;
                }
                //Si el perfil que está cambiando la contraseña es 'ADMIN' y la nueva contraseña no está considerada como 'strongest'
                    ...
                    ALERT.innerText = 'Como administrador, la contraseña de su cuenta debe ser la más segura';
                    e.preventDefault();
                    return;
                }
            });
