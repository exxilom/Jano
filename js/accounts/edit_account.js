    ...
    const messageFormError = messageError => {
        switch (messageError) {
            ...
            case 'name_account_incorrect':
                return 'nombre de cuenta invalido';
            case 'password_req':
                return 'contraseña requerida';
            case 'password_dang':
                return 'contraseña peligrosa';
        ...
        if (location.href.match(/accounts\.php\?edit=(\d)+$/)?.input !== undefined) {
            ...
            const getAccountsNameRepeat = info => {
                ...
                NAME_REPEAT.innerHTML = `
                    <div class="alert">Nombre de cuenta en uso</div>
                    <div class="scroll">
                        ${
             ...
             */
            const showErrors = () => {
                SPANS[1].innerText = PLATFORM_SELECT.value == "" ? "REQUERIDO" : ""
                NAME_ACCOUNT.classList = NAME_ACCOUNT.value == "" ? "input-error" : "input-correct";
                SPANS[2].innerText = NAME_ACCOUNT.value == "" ? "REQUERIDO" : "";
                SPANS[3].innerText = PASSWORD.value !== "" ? `${passManager.validatePasswordStrength(PASSWORD.value,[NAME_ACCOUNT.value])}` : "REQUERIDO";
            ...
            NAME_ACCOUNT.addEventListener("keyup", () => {
                ...
                if (NAME_ACCOUNT.value.replace(/\s/g,"").length && PASSWORD.value.length) 
                    ...
                    SPANS[3].innerText = SPANS[2].innerText = "NOMBRE DE CUENTA REQUERIDO";
            ...
            PASSWORD.addEventListener("keyup", () => {
                if (!NAME_ACCOUNT.value.replace(/\s/g,"").length) {
                    PASSWORD.value = "";
                    SPANS[2].innerText = "NOMBRE DE CUENTA REQUERIDO";
                    NAME_ACCOUNT.classList = 'input-error';
                    return;
                }
                DIV_OLD_PASS.classList = PASSWORD.value == currentPassword ? "alert" : "";
                DIV_OLD_PASS.innerText = PASSWORD.value == currentPassword ? "CONTRASEÑA DEMACIADO ANTIGUA" : "";
