<?php
    ...
    class Users extends DBAbstractModel {
        ...
        public function getUsers ($search = '') {
            if ($search == '')
                $this->query = "SELECT id, nick, email, perfil, current_state FROM jano_users WHERE perfil = 'USER'";
            elseif ($search == '*')
                $this->query = "SELECT id, nick, email, perfil, current_state FROM jano_users";
            else
                $this->query = "SELECT id, nick, email, perfil, current_state FROM jano_users 
                                WHERE perfil = 'USER' 
        ...
        public function getIdUserByNick ( $id = 0 ) {
            if ( $id !== 0 ) {
                $this->query = "SELECT id FROM jano_users WHERE nick = lower( :nick )";
        ...
        public function getUserByNick ( $nick = '' ) {
            if ( $nick !== '' ) {
                ...
                U.current_state,
                U.days_old_password
                FROM jano_users U, jano_keys K
                WHERE U.id = K.idUser and K.idCategory = 0 and U.id = (SELECT id FROM jano_users WHERE lower( nick ) = :nick)";
        ...
        public function getUserByEmail( $email = '' ) {
            if ( $email !== '' ) {
                $this->query = "SELECT * FROM jano_users WHERE email = lower( :email )";
        ...
        public function getUserById( $id ) {
            $this->query = "SELECT * FROM jano_users WHERE id = :id";
        ...
        public function setUser ( $user_data = array() ) {
            ...
            elseif ( $user_data['pass'] !== $user_data['pass2'])                //Si la contraseña y su verificación no coinciden
                ...
                $this->query = "INSERT INTO jano_users 
                                (
                                    jano_users.nick,
                                    jano_users.pass,
                                    jano_users.name,
                                    ...
                                    jano_users.email,
                                    jano_users.perfil,
                                    jano_users.current_state
                                ) 
                                VALUES (
                                    :nick,
                                    HEX(AES_ENCRYPT(:pass,:jano)),
                                    HEX(AES_ENCRYPT(:username,:jano)),
                                    HEX(AES_ENCRYPT(:surname,:jano)),
                                    :email,
                                    :perfil,
                                    :current_state
                                )";
                ...
                $this->parametros['jano']          = $generalKey;  //Obtenemos la llave genérica (llave idCategory == 0)
                //$this->parametros['jano']       = $user_data['jano'];
         ...
         */
        private function _setUserKeys( $idUser,$idCategory,$jano ) {
            $this->query = "INSERT INTO jano_keys (jano_keys.idUser, jano_keys.idCategory, jano_keys.password) 
                                VALUES (:idUser, :idCategory, :jano)";

            $this->parametros['idUser']      = $idUser;
            $this->parametros['idCategory']  = $idCategory;
            $this->parametros['jano']     = $jano;
        ...
        public function editUser ( $user_data = array() ) {
            ...
            $this->query = "UPDATE jano_users
                            SET nick = :nick, 
                            name = HEX( AES_ENCRYPT( :name,(SELECT password FROM jano_keys WHERE idUser = :id AND idCategory = 0) ) ), 
                            surname = HEX( AES_ENCRYPT( :surname, (SELECT password FROM jano_keys WHERE idUser = :id AND idCategory = 0) ) ), 
                            email = :email, 
        ...
        public function editPass ( $user_data = array() ) {
            ...
            $this->query = "UPDATE jano_users
                SET pass = HEX( AES_ENCRYPT( :pass,(SELECT password FROM jano_keys WHERE idUser = :id AND idCategory = 0) ) )
                WHERE id = :id";
        ...
        public function updateState( $newState, $idUser ) {
            ...
            $this->query = "UPDATE jano_users SET current_state = :new_state WHERE id = :id";
        ...
        public function deleteUser( $idUser ) {
            $this->query = "DELETE FROM jano_users WHERE id = :id";
