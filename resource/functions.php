<?php
    ...
    function renderTablePlatformList($categories, $result_search) {
        ...
        for ($i = 0; $i < sizeof($categories); $i++) { 
            ...
            echo "<table class='table-platform-list'>";
                echo "<tr><th>SUBCATEGORIAS</th><th>PLATAFORMAS</th><th>EDITAR</th><th>ELIMINAR</th></tr>";
    ...
    function renderTableRowsPlatform ($platformList) {
        ...
        for ($i = 0; $i < sizeof($platformList); $i++) {
            ...
            echo "<tr>";
                ...
                echo "<td><a href=".$_SERVER['PHP_SELF'].'?edit='.$platformList[$i]['id']."><button class='back'>EDITAR</button></a></td>";
                echo "<td><a href=".$_SERVER['PHP_SELF'].'?del='.$platformList[$i]['id']."><button class='cancel'>ELIMINAR</button></a></td>";
            echo "</tr>";
        }
    ...
    function renderUserList( $userList ) {
        echo "<table>";
            echo "<th>USUARIO</th><th>EMAIL</th><th>PERFIL</th><th>ESTADO</th><th>UPDATE</th><th>ELIMINAR</th>";
                    ...
                    echo "<tr>";
                        ...
                        if ( $user['perfil'] == "USER" ) {
                            echo ( ( ( $user['current_state'] == "PENDING" || $user['current_state'] == "BANNED" ) ) ? 
                                "<td><a href=".$_SERVER['PHP_SELF']."?activate=".$user['id']."><button class='accept'>ACTIVAR</button></a></td>" : 
                                "<td><a href=".$_SERVER['PHP_SELF']."?ban=".$user['id']."><button class='cancel'>BANEAR</button></a></td>" );
                            echo ( $user['current_state'] !== "ACTIVE" ? 
                                "<td><a href=".$_SERVER['PHP_SELF']."?del=".$user['id']."><button class='delete_user'>ELIMINAR</button></a></td>" :
                                "<td>---</td>" );
                        }
                        else {
                            echo "<td></td>";
